...
fontdetect = function()
{
	...
	// short hand utility function for queryselector
	...
	function _init ()
	{
		...
		// insert dummy element used for dummy text to apply fonts to 
		var helperHTMLDiv = '<div id="fontdetectHelper"><span>abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ</span></div>';
		var helperHTMLCSS = '<style> #fontdetectHelper { position: absolute; visibility: hidden; top: -200px; left: -100000px; width: 100000px; height: 200px; font-size: 200px; } </style>';

		insertElementBefore(helperHTMLDiv, 'body');
		insertElementBefore(helperHTMLCSS, 'head');

		function insertElementBefore(divAsString, tagName) {
			var HTML = qs(tagName).innerHTML;
			HTML = HTML.replace(/^/, divAsString);
		  qs(tagName).innerHTML = HTML;
		};
	}
	...
	return	{
		...
		isFontLoaded: function (p_cssFontName)
		{
			...
			{
				var helperSpan = qs('SPAN', qs('#fontdetectHelper'));
				helperSpan.style['font-family'] =  p_cssFontName + ',' + _aFallbackFonts[ix];
		...
		{
			var aStack = qs(p_element).style['font-family'];
